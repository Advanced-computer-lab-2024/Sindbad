...
describe("GET /tourist", () => {
	test("getTourist retrieves a tourist by ID", async () => {
      ...
      test("getTourist returns 404 for non-existent tourist", async () => {
        Tourist.findById.mockResolvedValueOnce(null);
      
        const req = { params: { id: 2 } };
        const res = { status: jest.fn().mockReturnThis(), send: jest.fn() };
      
        await getTourist(req, res);
      ...
      test("getTourist handles errors", async () => {
